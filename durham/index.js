...
infectedGraph.titletext = "County Durham: Trend in Infected";

// Plots the infection graph
infectedGraph.plot("bar", "durham-infected.csv");
...
deathGraph.titletext = "County Durham: Trend in Deaths";

// Plots the death graph
deathGraph.plot("bar", "durham-deaths.csv");
...
hospitalGraph.titletext = "County Durham: Hospital Occupancy Due to Covid-19";

// Plots the hospital graph
hospitalGraph.plot("bar", "durham-hospitalised.csv");
...
let map = L.map('durham-map').setView([54.7, -1.7], 9);

// Plots the map
plotMap("county-durham-map.geojson").catch(error => {
    console.log(error);
});
