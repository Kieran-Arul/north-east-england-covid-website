...
infectedGraph.titletext = "Darlington: Trend in Infected";

// Plots the infection graph
infectedGraph.plot("bar", "darlington-infected.csv");
...
deathGraph.titletext = "Darlington: Trend in Deaths";

// Plots the death graph
deathGraph.plot("bar", "darlington-deaths.csv");
...
hospitalGraph.titletext = "Darlington: Hospital Occupancy Due to Covid-19";

// Plots the hospital graph
hospitalGraph.plot("bar", "darlington-hospitalised.csv");
...
let map = L.map('darlington-map').setView([54.6, -1.5595], 9.5);

// Plots the map
plotMap("darlington-map.geojson").catch(error => {
    console.log(error);
});
